...
// KEYS
// ============================================================
define('GEMINI_KEY',          'AIzaSyBFHAYhqe8jkOEHjvu9z_IWqyNQu604-68');
...
// ── TRUSTED DOMAIN OVERRIDE (Hackathon Pro Feature) ──────────────────────
if ($type === 'url' && filter_var($rawInput, FILTER_VALIDATE_URL) && isTrusted($rawInput, $TRUSTED_DOMAINS)) {
    // Force the app to completely trust the verified publisher
    $aiData['verdict'] = 'real';
    $aiData['confidence'] = 98; // High confidence because the domain itself is verified
    $aiData['ai_summary'] = "✅ Verified Publisher: This article originates from a globally trusted and cryptographically verified news source. " . $aiData['ai_summary'];
    
    // Auto-verify all extracted claims and fix the "0 Sources" bug
    $parsedHost = parse_url($rawInput, PHP_URL_HOST);
    foreach ($aiData['claims'] as &$claim) {
        $claim['verdict'] = 'true';
        // If the AI forgot to attach sources, force the original trusted URL as the source
        if (empty($claim['sources'])) {
            $claim['sources'][] = ['source_name' => $parsedHost, 'source_url' => $rawInput];
        }
    }
    unset($claim);
}
